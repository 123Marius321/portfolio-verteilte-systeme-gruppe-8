...
export const updateSupplierValidators = [
  ...
  check("delivery").isLength( { min: 1 } ).withMessage("invalid delivery - valid input: at least one entry"),
  ...
  check("deliveryDay").custom((value, { req }) => {
    if (value != "Montag" && value != "Dienstag" && value != "Mittwoch" && value != "Donnerstag" && value != "Freitag" && value != "Samstag" && value != "Sonntag") {
      return false;
    }
    return true;
  }).withMessage('invalid deliveryDay - valid input: Montag, Dienstag, Mittwoch, Donnerstag, Freitag, Samstag, Sonntag')
];
